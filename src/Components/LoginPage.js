...
import { FaEnvelope, FaLock, FaCompass } from 'react-icons/fa';
    ...
    if (validateForm()) {
      try {
        ...
        if (error.response.status === 400) {
          setErrors({ ...errors, general: 'Invalid credentials. You need to sign up first.' });
        } else {
        ...
        <Row className="justify-content-center">
          <Col md={6}>
            ...
            >
              ...
              <p className="text-center mb-4 text-white" style={{ opacity: 0.6, fontSize: '0.9rem' }}>
                Log in to access your architectural blueprints
              </p>
              <AnimatePresence>
                {showSuccess && (
                  <motion.div
